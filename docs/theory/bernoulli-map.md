...
## Confusion Matrix Scales

**Small function spaces** (like `bool -> bool` with 4 functions): We can write the full confusion matrix and analyze it completely.

**Large function spaces** (like `X -> Y` with `|Y|^|X|` functions): The confusion matrix becomes exponentially large. However, **structural patterns** imposed by specific algorithms still determine:
- **Sparsity structure**: Which function transitions are impossible
- **Rank properties**: Whether the structure enhances or reduces inferability  
- **Information limits**: What can theoretically be recovered about the latent function
