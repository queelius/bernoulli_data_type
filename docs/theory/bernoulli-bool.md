...
## Rank vs Order Analysis

The confusion matrix rank reveals deeper structure than parameter count alone:

**First-order model** (symmetric: ε = error rate for both true/false):
```
...
| false           | ε     | 1-ε   |
```
- **Order**: 1 (single parameter ε)
- **Rank**: 2 when ε ≠ 0.5, rank 1 when ε = 0.5  
- **Inference**: Impossible when ε = 0.5 (maximum entropy)

**Second-order model** (asymmetric: η = false negative rate, ε = false positive rate):
```
| latent\observed | true  | false |
|-----------------|-------|-------|
| true            | 1-η   | η     |
| false           | ε     | 1-ε   |
```
- **Order**: 2 (parameters ε, η)
- **Rank**: 2 when ε + η ≠ 1, rank 1 when ε + η = 1
- **Inference**: Impossible when ε + η = 1 (doubly stochastic)

**Key insight**: Higher order doesn't guarantee better inference. Rank determines the fundamental recoverability of latent values through Bayesian inference.

## Information-Theoretic Analysis

Entropy captures the fundamental information content and uncertainty in Bernoulli approximations:

**Conditional Entropy H(latent|observed):**

For first-order model with error rate ε:
- When observing `true`: H(latent|observed=true) depends on prior and ε
- When observing `false`: H(latent|observed=false) depends on prior and ε  
- **Maximum entropy case**: ε = 0.5 → H(latent|observed) = H(latent) (no information gain)

**Matrix Entropy H(confusion matrix):**
The entropy of the confusion matrix itself measures uncertainty about the approximation process:

```cpp
H = -∑ᵢ∑ⱼ qᵢⱼ log(qᵢⱼ)
```

For symmetric case (ε ≠ 0.5): H = -2[(1-ε)log(1-ε) + ε log(ε)]
For maximum entropy case (ε = 0.5): H = -4[0.5 log(0.5)] = 2 bits

**Information Hierarchy:**
1. **Unconstrained**: Any 2×2 stochastic matrix → maximum entropy
2. **Structural constraints**: "Symmetric channel" → reduced entropy  
3. **Parameter knowledge**: Known ε → further entropy reduction
4. **Observed values**: Given observation → conditional entropy

**Example - Bloom Filter Structure:**
Standard 2×2 Boolean matrix: many possible structures
Bloom filter constraint: no false negatives → entropy reduced
Known false positive rate ε → entropy = -[ε log(ε) + (1-ε)log(1-ε)]
