...
namespace alex::approximate_set
{
    ...
    public:
        ...
        BloomFilter(const BloomFilterParameters& p)
            : bit_table_(nullptr),
        ...
        BloomFilter(const BloomFilter& f)
        {
        ...
        double effective_fpp() const
        {
            ...
            */
            return std::pow(1. - std::exp(-1. * salt_.size() * inserted_element_count_ / size()), 1. * salt_.size());
        }
    ...
    protected:
        ...
        bloom_type hash_ap(const unsigned char* begin, size_t remaining_length, bloom_type hash) const
        {
