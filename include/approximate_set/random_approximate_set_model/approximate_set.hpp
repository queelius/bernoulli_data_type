 ...
 *
 * approximate_set<T> uses type-erasure and generic programming to
 * give value-semantics to objects that models approximate sets.
 * 
 * Suppose ASet is an object type that models approximate_set<T>. Then, three function overloads
 * must be in scope:
 *   is_member(T x, ASet a)      : returns true if x in a,
 *   false_positive_rate(ASet a) : returns the false positive rate of a as a real interval,
 *   true_positive_rate(ASet a)  : returns the true positive rate of a as a real interval.
 * 
 * If these are in scope, an instance a of ASet may be wrapped into an approximate_set<T>
 * with the notation approximate_set_wrap(a). You may compose any such objects with set-theoretic
 * operations like union and complement.
 */
...
namespace alex::approximate_set
{
    ...
    public:
        ...
        approximate_set(empty) : self_(std::make_shared<approximate_set_empty>()) {};
        approximate_set() : self_(std::make_shared<approximate_set_empty>()) {};
        approximate_set(universal) : self_(std::make_shared<approximate_set_universal>()) {};
