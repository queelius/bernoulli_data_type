...
One way a particular value of this sum type may be constructed is \mintinline{c++}{variant<char,bool>('1')}.
...
\begin{example}
	...
	If the invariant is that the denominator is not $0$, then a \emph{value constructor} \texttt{rational} $\colon$ \texttt{int} $\times$ \texttt{int} $\mapsto$ \texttt{optional\textless Rational\textgreater} takes a numerator and denominator and outputs either a rational or, if the invariants are violated, \emph{nothing}.\footnote{Alternatively, the value constructor can be a \emph{partial function} $\mathtt{rational} \colon \mathtt{int} \times \mathtt{int} \pfun \mathtt{Rational}$ that is undefined for input $\Pair{x}{0}$ for any $x \in \mathtt{int}$.}
...
{
	if (denom == 0)
	return {}; // Return the value that denotes $\mathrm{Nothing}$.
...
This may not seem particularly useful, but it permits space-efficient representations and, moreover, concepts like \emph{cipher algebraic data types} may be based on it with some notion of closure.
...
Given a function $\Fun{f} \colon X \to Y$, applying $\Fun{f}$ to an approximate value of type $X$ yields an approximate value of type $Y$, as described in \cref{sec:error_prop}.
...
The function $\mathtt{fmap} \colon (\mathtt{bool} \to \mathtt{bool}) \times \mathtt{approx\textless bool\textgreater} \to \mathtt{approx\textless bool\textgreater}$ is defined as
\begin{minted}{c++}
...
However, we could estimate the result using Monte Carlo simulation.
...
The same principle applies at the type level: $\AT{\Set{X}}[\fprate][\fnrate]$ may be erased to $\AT{\Set{X}}$, which may be erased to $\Set{X}$.
